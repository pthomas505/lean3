...
def eval_term (D : Type) (m : interpretation D) (v : assignment D) : term → D
| (var x) := v.var x
| (func n f t) := m.func n f (fun (i : fin n), eval_term (t i))
| (func_var n u t) := v.func_var n u (fun (i : fin n), eval_term (t i))


def holds (D : Type) (m : interpretation D) : assignment D → formula → Prop
| _ bottom := false
| _ top := true
...
| v (exists_var x φ) := ∃ a : D,
		holds {
			...
			func_var := v.func_var,
			pred_var := v.pred_var } φ
| v (forall_pred_var n X φ) := ∀ a : (pred_var_symbol → (fin n → D) → Prop),
		holds {
			var := v.var,
			func_var := v.func_var,
			pred_var := fun (k : ℕ), function.update v.pred_var n a k } φ

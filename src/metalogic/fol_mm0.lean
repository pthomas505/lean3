...
begin
  ...
  {
		unfold formula.scoped_in_env,
	},
  case formula.not_ : φ φ_ih
  {
		unfold formula.scoped_in_env at *,
		exact φ_ih h2,
	},
  ...
  {
		apply exists.elim h1, intros E1 a1,
		unfold formula.scoped_in_env at h2,
		apply exists.elim h2, intros d a2,
		cases a2,
		unfold formula.scoped_in_env,
		rewrite a1,
		...
		split,
		{
			...
			apply or.intro_right, exact a2_left,
		},
		{
			exact a2_right,
		},
...
begin
  ...
  case formula.def_ : name args
  {
		...
		{
			cases h,
			rewrite h_left at a2_right_left,
			rewrite h_right at a2_right_right,
			rewrite a1 at h3,
			simp only [list.pairwise_cons] at h3,
			cases h3,
			by_contradiction contra,
			apply h3_left d' a2_left a2_right_left a2_right_right,
		},
		{
			refl,
		},
